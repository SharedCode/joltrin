...
| `execute_step` | The barrier certificate itself. Runs the same check; only commits the step to the trace if it passes, returns `ExecuteStepResult{executed, step, trace, blocked}`. |
